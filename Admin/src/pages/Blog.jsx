...
const BlogAdminPanel = () => {
  ...
  const [settings, setSettings] = useState({
    siteTitle: "Pacific Travel Blog",
    postsPerPage: 8,
    defaultStatus: POST_STATUS.DRAFT,
  ...
  const stats = useMemo(() => {
    ...
    const published = posts.filter(
      (p) => p.status === POST_STATUS.PUBLISHED,
    ).length;
  ...
  const toggleSelectAll = () =>
    setSelectedIds((prev) =>
      prev.length === paginated.length ? [] : paginated.map((p) => p._id),
    );
  ...
  const handleSubmit = (e) => {
    ...
    const published_at =
      formData.status === POST_STATUS.PUBLISHED
        ? selectedPost?.published_at || now
        : null;
    if (selectedPost) {
      setPosts((prev) =>
        prev.map((p) =>
          p._id === selectedPost._id
            ? { ...p, ...formData, slug, published_at, updatedAt: now }
            : p,
        ),
      );
      ...
      const newPost = {
        ...
        published_at,
        view_count: 0,
        images_in_content: formData.images_in_content,
        createdAt: now,
        updatedAt: now,
      };
  ...
  const handleEdit = (post) => {
    ...
    setFormData({
      ...
      images_in_content: post.images_in_content || [],
    });
    setPreviewMode("write");
    setView("editor");
  };
  ...
  const handleCatSubmit = (e) => {
    ...
    if (editingCat) {
      setCategories((prev) =>
        prev.map((c) =>
          c._id === editingCat._id ? { ...c, ...catForm, slug } : c,
        ),
      );
      ...
      setCategories((prev) => [
        ...prev,
        { _id: "c" + Date.now(), ...catForm, slug, postCount: 0 },
      ]);
  ...
  const lineChartData = {
    ...
    datasets: [
      {
        label: "Published",
        data: [3, 5, 4, 7, 6, 5, 8, 4],
        borderColor: "#4f46e5",
        backgroundColor: (ctx) => {
          const c = ctx.chart;
          const { ctx: cx, chartArea: a } = c;
          if (!a) return null;
      ...
      {
        label: "Views (K)",
        data: [1.2, 2.5, 1.8, 4.2, 3.5, 2.8, 5.1, 3.2],
        borderColor: "#059669",
        borderWidth: 2,
        borderDash: [6, 4],
        tension: 0.4,
    ...
    datasets: [
      {
        data: [
          stats.published,
          stats.drafts,
          posts.filter((p) => p.status === POST_STATUS.ARCHIVED).length,
        ],
  ...
  const viewTitles = {
    dashboard: ["BLOG", "Overview of your content"],
    posts: ["All Posts", `${posts.length} articles total`],
    editor: [
    ...
    categories: ["Categories", "Manage blog categories"],
    settings: ["Settings", "Configure blog preferences"],
  };

  const getAuthorName = (id) =>
    MOCK_AUTHORS.find((a) => a._id === id)?.name || "Unknown";
    ...
    <div className="ba-admin-root">
      <main className="ba-main-full">
        <div className="ba-topbar">
          ...
          {/* Top Navigation Tabs */}
          <div
            className="ba-top-nav"
            style={{ flex: 1, display: "flex", alignItems: "center" }}
          >
            {navItems.map((item) => (
              <button
                key={item.id}
                className={`ba-tab-item ${view === item.id ? "active" : ""}`}
                onClick={() => {
                  if (item.id === "editor") {
                    resetForm();
                  }
              ...
              >
                ...
                {item.badge !== undefined && (
                  <span className="ba-tab-badge">{item.badge}</span>
                )}
              </button>
            ))}
          </div>
          ...
          <div className="ba-topbar-actions">
            {/* ✅ Fixed Refresh Button — single button, clean placement */}
            <button
              type="button"
            ...
            {view === "editor" && (
              ...
              >
                <FiArrowLeft size={16} /> Back to Posts
              </button>
            )}
          </div>
        </div>
        ...
        <div className="ba-content">
                ...
                <>
                  <div className="ba-stats-grid">
                    <div className="ba-stat-card accent">
                      <div>
                        <div className="ba-stat-label">Total Posts</div>
                        <div className="ba-stat-value">{stats.total}</div>
                        <div className="ba-stat-change positive">↑ 12%</div>
                      </div>
                      <div className="ba-stat-spark">
                        <Line
                          data={sparkData(
                            [3, 5, 4, 7, 6, 8, 10],
                            "rgb(99,102,241)",
                    ...
                    <div className="ba-stat-card success">
                      <div>
                        <div className="ba-stat-label">Published</div>
                        <div className="ba-stat-value">{stats.published}</div>
                        <div className="ba-stat-change positive">↑ 8%</div>
                      </div>
                      <div className="ba-stat-spark">
                        <Line
                          data={sparkData(
                    ...
                    <div className="ba-stat-card warning">
                      <div>
                        <div className="ba-stat-label">Drafts</div>
                        <div className="ba-stat-value">{stats.drafts}</div>
                        <div className="ba-stat-change negative">↓ 3%</div>
                      </div>
                      <div className="ba-stat-spark">
                        <Line
                          data={sparkData(
                    ...
                    <div className="ba-stat-card info">
                      <div>
                        <div className="ba-stat-label">Total Views</div>
                        <div className="ba-stat-value">
                          {stats.totalViews.toLocaleString()}
                        </div>
                        <div className="ba-stat-change positive">↑ 24%</div>
                      </div>
                      <div className="ba-stat-spark">
                        <Line
                          data={sparkData(
                  ...
                  <div className="ba-charts-grid">
                    <div className="ba-card">
                      ...
                      <div className="ba-chart-wrapper">
                        <Line data={lineChartData} options={lineOpts} />
                      </div>
                    </div>
                    <div className="ba-card">
                      <div className="ba-card-header">
                        <span className="ba-card-title">Post Status</span>
                      </div>
                      <div className="ba-chart-wrapper">
                  ...
                  <div className="ba-card">
                    <div className="ba-card-header">
                      <span className="ba-card-title">Recent Posts</span>
                      <button
                      ...
                      {posts.slice(0, 5).map((post) => (
                        ...
                        >
                          ...
                          <div className="ba-recent-info">
                            ...
                            <div className="ba-recent-meta">
                              {getAuthorName(post.author)} ·{" "}
                              {formatDate(post.published_at || post.createdAt)}{" "}
                              · {post.view_count.toLocaleString()} views
                            </div>
                          </div>
                          <span className={`ba-status-badge ${post.status}`}>
              ...
              <div style={{ padding: "1.5rem 1.5rem 0" }}>
                {selectedIds.length > 0 && (
                  <div className="ba-bulk-bar">
                    <span className="ba-bulk-count">
                      {selectedIds.length} selected
                    </span>
                    <button
                      className="ba-btn ba-btn-sm ba-btn-success"
                      onClick={() => handleBulkStatus(POST_STATUS.PUBLISHED)}
                    >
                      <FiCheck size={14} /> Publish
                    </button>
                    <button
                      className="ba-btn ba-btn-sm ba-btn-ghost"
                      onClick={() => handleBulkStatus(POST_STATUS.DRAFT)}
                    >
                      Set Draft
                    </button>
                    <button
                ...
                <div className="ba-toolbar">
                  <div className="ba-toolbar-left">
                    <div className="ba-search-box">
                      <FiSearch className="ba-search-icon" />
                      <input
                    ...
                    <div className="ba-filter-pills">
                      {[
                        ...
                        <button
                          key={s}
                          className={`ba-filter-pill ${filterStatus === s ? "active" : ""}`}
                          onClick={() => {
                            setFilterStatus(s);
                            setPage(1);
                          }}
                  ...
                  <thead>
                    <tr>
                      ...
                      <th
                        ...
                        className={sortField === "view_count" ? "sorted" : ""}
                      >
                        Views{" "}
                        <SortIcon
                          field="view_count"
                      ...
                      <th
                        ...
                        className={sortField === "createdAt" ? "sorted" : ""}
                      >
                        Date{" "}
                        <SortIcon
                          field="createdAt"
                        ...
                        <tr key={post._id}>
                          ...
                          <td data-label="Title">
                            <div className="ba-post-cell">
                              <span className="ba-post-cell-title">
                                {post.title}
                              </span>
                              {post.excerpt && (
                                <span className="ba-post-cell-excerpt">
                                  {post.excerpt}
                                </span>
                              )}
                              <span className="ba-post-cell-slug">
                                /{post.slug}
                              </span>
                              {post.tags?.length > 0 && (
                                <div className="ba-tags-row">
                                  {post.tags.slice(0, 3).map((t) => (
                                    <span key={t} className="ba-tag-pill">
                                      {t}
                                    </span>
                                  ))}
                                  {post.tags.length > 3 && (
                                    <span className="ba-tag-pill">
                                      +{post.tags.length - 3}
                                    </span>
                                  )}
                                </div>
                          ...
                          <td data-label="Featured">
                            {post.is_featured && (
                              <span className="ba-featured-star">
                                <FiStar fill="#f59e0b" />
                              </span>
                            )}
                          </td>
                          <td data-label="Actions">
                            <div className="ba-actions-cell">
                              ...
                              <button
                                ...
                                onClick={() => setDeleteDialog(post._id)}
                              >
                                <FiTrash2 size={13} />
                              </button>
                            </div>
              ...
              {totalPages > 1 && (
                ...
                >
                  <span className="ba-pagination-info">
                    ...
                    {Math.min(page * perPage, processed.length)} of{" "}
                    {processed.length}
                  </span>
                  <div className="ba-pagination-btns">
                    ...
                    {Array.from({ length: totalPages }, (_, i) => i + 1).map(
                      (n) => (
                        <button
                          key={n}
                          className={`ba-page-btn ${page === n ? "active" : ""}`}
                          onClick={() => setPage(n)}
                        >
                          {n}
                        </button>
                ...
                <div className="ba-form-main">
                  <div className="ba-form-section">
                    <input
                      type="text"
                      name="title"
                      className="ba-input ba-input-title"
                    ...
                    />
                    <div
                      className="ba-char-count"
                      style={{
                        display: "flex",
                        justifyContent: "space-between",
                      }}
                    >
                      <span className="ba-slug-preview">
                        Slug: <span>/{slugify(formData.title) || "..."}</span>
                      </span>
                      <span
                        className={
                          formData.title.length > 180
                            ? formData.title.length > 195
                              ? "at-limit"
                              : "near-limit"
                            : ""
                        }
                  ...
                  <div className="ba-form-section">
                    <div className="ba-form-section-title">
                      <FiFileText className="icon" /> Content
                    </div>
                    <div className="ba-preview-toggle">
                      <button
                        type="button"
                        className={`ba-preview-tab ${previewMode === "write" ? "active" : ""}`}
                      ...
                      <button
                        type="button"
                        className={`ba-preview-tab ${previewMode === "preview" ? "active" : ""}`}
                    ...
                    {previewMode === "write" ? (
                      <textarea
                        ...
                        className="ba-textarea ba-textarea-content"
                  ...
                  <div className="ba-form-section">
                    ...
                    <textarea
                      name="excerpt"
                      className="ba-textarea ba-textarea-sm"
                    ...
                    />
                    <div
                      className={`ba-char-count ${formData.excerpt.length > 270 ? (formData.excerpt.length > 290 ? "at-limit" : "near-limit") : ""}`}
                    >
                  ...
                  <div className="ba-form-section">
                    <div className="ba-form-section-title">
                      <FiGlobe className="icon" /> SEO Settings
                    </div>
                    <div className="ba-form-group">
                      ...
                      />
                      <div
                        className={`ba-char-count ${formData.meta_title.length > 60 ? "near-limit" : ""}`}
                      >
                    ...
                    <div className="ba-form-group">
                      ...
                      />
                      <div
                        className={`ba-char-count ${formData.meta_description.length > 140 ? "near-limit" : ""}`}
                      >
                ...
                <div className="ba-form-sidebar">
                  <div className="ba-form-section">
                    ...
                    <div className="ba-form-actions">
                      <button type="submit" className="ba-btn ba-btn-publish">
                        {selectedPost ? "✓ Update Article" : "🚀 Publish"}
                      </button>
                    </div>
                    <div className="ba-form-actions">
                      <button
                        type="button"
                        className="ba-btn ba-btn-draft"
                        onClick={() => {
                          setFormData((prev) => ({
                            ...prev,
                  ...
                  <div className="ba-form-section">
                    <div className="ba-form-section-title">
                      <FiImage className="icon" /> Featured Image
                    </div>
                    <div className="ba-form-group">
                      ...
                      <div className="ba-image-upload-wrapper">
                        <label
                          ...
                          className="ba-image-drop"
                        >
                          <FiUpload className="ba-image-drop-icon" />
                          <p>Click or drag image here to upload</p>
                          <span className="ba-sub-text">
                      ...
                      <div className="ba-separator">
                        <span>OR</span>
                      </div>
                      <input
                        type="url"
                        name="featured_image.url"
                        className="ba-input"
                    ...
                    {formData.featured_image.url && (
                      <div className="ba-image-preview">
                        ...
                        <button
                          type="button"
                          className="ba-image-preview-remove"
                          onClick={() =>
                            setFormData((prev) => ({
                              ...prev,
                              featured_image: {
                    ...
                    <div
                      className="ba-form-group"
                      style={{ marginTop: "0.75rem" }}
                    >
                      <label className="ba-label">Alt Text</label>
                      <input
                        type="text"
                        name="featured_image.alt"
                        className="ba-input"
                        ...
                        maxLength={125}
                      />
                      <div className="ba-char-count">
                        {formData.featured_image.alt.length}/125
                      </div>
                    </div>
                    <div className="ba-form-group">
                  ...
                  <div className="ba-form-section">
                    <div className="ba-form-section-title">
                      <FiTag className="icon" /> Categories
                    </div>
                  ...
                  <div className="ba-form-section">
                    ...
                    <div className="ba-tags-chips">
                      {formData.tags.map((tag) => (
                        <span key={tag} className="ba-tag-chip">
                          {tag}
                          <button
                            type="button"
                            className="ba-tag-chip-remove"
                    ...
                    <span className="ba-hint">
                      Press Enter to add a tag (max 50 chars each)
                    </span>
                  </div>
                  ...
                  {formData.images_in_content?.length > 0 && (
                    <div className="ba-form-section">
                      <div className="ba-form-section-title">
                        <FiImage className="icon" /> Content Images
                      </div>
          ...
          {view === "categories" && (
            <div className="ba-category-grid">
              ...
              <div className="ba-card">
                <div className="ba-card-header">
                  <span className="ba-card-title">All Categories</span>
                </div>
          ...
          {view === "settings" && (
            <div className="ba-settings-grid">
              <div className="ba-card">
                <div className="ba-card-header">
                  <span className="ba-card-title">General Settings</span>
                </div>
                <div className="ba-form-group">
                ...
                <div className="ba-form-group">
                  <label className="ba-label">Posts Per Page</label>
                  <input
                    type="number"
                    className="ba-input"
                    ...
                    onChange={(e) =>
                      setSettings((prev) => ({
                        ...prev,
                        postsPerPage: parseInt(e.target.value) || 8,
                      }))
                    }
                    min={1}
                    max={50}
                ...
                <div className="ba-form-group">
                  <label className="ba-label">Default Post Status</label>
                  <select
                    className="ba-select"
              ...
              <div className="ba-card">
                <div className="ba-card-header">
                  <span className="ba-card-title">Comment Settings</span>
                </div>
                <div className="ba-toggle-group">
                  <span className="ba-toggle-label">Enable Comments</span>
                  <label className="ba-toggle">
                    <input
                      type="checkbox"
                ...
                <div className="ba-toggle-group">
                  <span className="ba-toggle-label">
                ...
                <button
                  className="ba-btn ba-btn-primary"
                  onClick={() => showToast("Comment settings saved!")}
                  style={{ marginTop: "1rem" }}
                >
                  Save
                </button>
      ...
      {toast && (
        <div className={`ba-toast ${toast.type}`}>
