...
export default function FeedbackPage() {
  ...
  const graphData = useMemo(() => {
    ...
    if (graphRange === "week") {
      const buckets = DAY_NAMES.map((label) => ({
        label,
        Positive: 0,
        Negative: 0,
      }));
    ...
    return buckets.map(({ label, Positive, Negative }) => ({
      label,
      Positive,
      Negative,
    }));
  ...
  // ─── Core fetch functions (take no state deps — always stable) ───
    ...
    try {
      ...
      setFeedbacks(
        items.map((r) => ({
          ...
          packageName:
            r.packageName || r.package?.title || r.packageTitle || "",
          rating: r.rating || 0,
  ...
  useEffect(() => {
    ...
    fetchReviews();
    fetchRatingsSummary();
  }, [fetchReviews, fetchRatingsSummary, navigate]);
  ...
  const handleRefresh = async () => {
    ...
    setFeedbacks([]);
    setRatings({ overall: 0, label: "", from: 0, items: [] });
    ...
    try {
      ...
      await Promise.all([fetchReviews(), fetchRatingsSummary()]);
    } catch (err) {
  ...
  return (
    <div
      className="fb-page-main-wrapper"
      style={{
        display: "flex",
            ...
            <div className="row g-3 mt-1">
              {isRefreshing ? (
                <div className="col-12">
                  <div className="fb-empty">Refreshing feedback...</div>
                </div>
              ) : (
                pageCards.map((f) => (
              ...
              {loading && pageCards.length === 0 && !isRefreshing && (
                <div className="col-12">
                  <div className="fb-empty">Loading reviews...</div>
                </div>
              )}
              {!loading && !isRefreshing && pageCards.length === 0 && (
                <div className="col-12">
